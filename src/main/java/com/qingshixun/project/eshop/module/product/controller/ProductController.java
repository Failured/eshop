...
public class ProductController extends BaseController {
    ...
    public String list(Model model, @RequestParam Long categoryId) {
    	if (categoryId==null) {
			List<ProductDTO>productDTOsd=productService.getselect();
		}
    ...
    public String main(Model model, @RequestParam Long productId) {
        ...
        model.addAttribute("total", evaluateService.getEvaluateCountByProductif(productId));
        ...
        return new SimpleHandler(request) {
            ...
            protected void doHandle(ResponseData responseData) throws Exception {
                   if (evaluate.getScore()<=2) {
                	   evaluate.setEvaluateStatus("不满意");	
				     }else if (evaluate.getScore()==3) {
				    	 evaluate.setEvaluateStatus("一般");	
					}else {
						 evaluate.setEvaluateStatus("满意");	
					      }
