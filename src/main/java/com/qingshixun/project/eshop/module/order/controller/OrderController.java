...
public class OrderController extends BaseController {
    ...
    public String orderList(Model model,@RequestParam(defaultValue = "1")int pageno) {
        ...
        // 传递我的订单数据
        model.addAttribute("orders", orderService.getOrdersByMember(member.getId(),pageno));
        // 传递登录会员数据
    ...
    @RequestMapping(value = "/delect/{orderId}", method = RequestMethod.GET)
    public String orderDetaildt(Model model, @PathVariable Long orderId,@RequestParam(defaultValue = "1")int pageno) {
        MemberDTO member = this.getCurrentUser();
        //将id传入进行删除
        orderItemService.getOrderItemsByOrderda(orderId);
        // 传递商品分类数据
        model.addAttribute("productCategories", productCategoryService.getProductCategories());
        // 传递我的订单数据
        model.addAttribute("orders", orderService.getOrdersByMember(member.getId() ,pageno));
