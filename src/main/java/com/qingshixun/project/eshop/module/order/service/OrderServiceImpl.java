...
import com.qingshixun.project.eshop.util.pageinfo;
...
public class OrderServiceImpl extends BaseService {
     ...
     * @param pageno 
     */
    public pageinfo<OrderDTO> getOrdersByMember(Long memberId, int pageno) {
    	pageinfo<OrderDTO>oredr=new pageinfo<OrderDTO>();
        ...
        oredr.setData(orders);
        oredr.setPageno(pageno);
        oredr.setTotalpages(query());
        return oredr;
    }
    
    //获取订单总数
    public int query(){
		int goodscount=orderDao.getOrderd();
		int totalpages=goodscount%5==0? goodscount/5 : goodscount/5+1;
		return totalpages;
